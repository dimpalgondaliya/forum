...
function create_forum($parent_forum_id, $new_forum){
    ...
    */
    ...
    $query = 'INSERT INTO `phpbb_forums` (`parent_id`, `left_id`, `right_id`,`forum_parents`, `forum_name`, `forum_desc`, `forum_type`, `forum_status`, `display_subforum_list`, `display_on_index`, `enable_icons`, `forum_flags` ,`forum_rules` ) Values (0, '.$next_leftid.', '.$next_rightid.', "" ,"'.$new_forum.'", "", 0, 0, 1, 1, 1, 32,"");';
    //echo $query.'  ';
    $result = $db->query($query);
    if ($result)
    ...
    //echo '<b>'.$new_forum.' has forum_id '.$new_forum_id.'</b>.<p>';
    return $new_forum_id;
}
